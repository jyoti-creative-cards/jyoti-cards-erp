...
def compute_bill_totals(
    ...
    n = len(raw_lines)
    has_item_discounts = any(lr["item_discount_pct"] > 0 for lr in raw_lines)
    ...
    for lr in raw_lines:
        ...
        if item_dp > 0:
            ...
            ld = (li / subtotal_inclusive * overall_discount_amount).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        else:
            ld = Decimal("0")
        line_discounts.append(ld)

    if n > 0 and not has_item_discounts and overall_discount_amount > 0:
        # Correct rounding drift so line discounts sum to overall discount
        allocated = sum(line_discounts)
    ...
    # Always roll up from lines so per-line and overall both populate discount_amount
    ...
    for lr, ld in zip(raw_lines, line_discounts):
        ...
        if gst_enabled and r > 0:
            ...
            taxable_unit = (line_taxable / Decimal(qty)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP) if qty > 0 else Decimal("0")
        else:
