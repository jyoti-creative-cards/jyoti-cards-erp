...
def list_payment_modes(
    ...
    # Anyone who can actually record a payment needs to be able to list modes
    # to pick from — finance.write alone locked out ap.write/ar.write-only staff,
    # who'd then silently see "no payment modes" (see finance.js fetch fallback).
    auth: AuthContext = Depends(require_any_permission("finance.write", "ap.write", "ar.write")),
):
