...
const Stock = (() => {
  ...
  async function renderWizard() {
    ...
    if (wizardStep === 2 && wizardMode === "bill_received") {
      ...
      setStockWizardChrome("Bill Order", "Step 2 — billed qty and vendor bill");
      bodyEl.innerHTML = `
        <div class="vo-wiz-step-head">
          <h4>${ctx.esc(placedOrder.vendor_label)}</h4>
          <p>Billed qty starts from received (editable). Then enter bill details.</p>
        </div>
        <div class="stock-receive-table-wrap">
          <table class="data stock-receive-table"><thead><tr>
            <th></th><th>Product</th><th>Category</th><th>Received</th><th>Price</th><th>Billed</th>
          </tr></thead><tbody>
            ${wizardLines.map((l, i) => {
              const img = (l.image_urls && l.image_urls[0]) || "";
              return `<tr>
    ...
    if (wizardStep === 3 && wizardMode === "bill_received") {
      setStockWizardChrome("Debit Notes", "Adjust payable if bill and goods differ");
  ...
  function saveReceiptMeta() {
    ...
    if (totalEl) receiptMeta.totalBilledAmount = totalEl.value || "";
  ...
  function renderDebitNoteStep(bodyEl, footerEl) {
    ...
    bodyEl.innerHTML = `
      ...
      ${pendingDebitNotes.length
        ...
        : HubUI.emptyState({ title: "No debit notes yet", sub: "Skip if the bill matches the goods." })}
  ...
  async function submitReceipt() {
    ...
    try {
      ...
      if (!isReceive && (!totalBilled || totalBilled <= 0)) return ctx.toast("Enter total bill amount", "error");
